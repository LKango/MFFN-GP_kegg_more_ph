...
def generate_data(features,compounds):
    ...
    rootx = "./test1/mlp_data/sub_channel1/"
    rooty = "./test1/mlp_data/pro_channel1/"
    ...
    for key1, value1 in list(substrates_dict.items()):
        sub_channel = np.zeros((1, PubChem))
        temp_channel = np.zeros((1, PubChem))
    ...
    for key2, value2 in list(products_dict.items()):
        ...
        pro_channel = np.zeros((max, PubChem))
        temp_channel = np.zeros((1, PubChem))
        if len(value2) < 10:
