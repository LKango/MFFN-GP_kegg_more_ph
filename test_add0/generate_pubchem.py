...
def generate_data(features,compounds):
    ...
    rootx = "./test1/data/sub_channel1/"
    rooty = "./test1/data/pro_channel1/"
    ...
    for key1, value1 in list(substrates_dict.items()):
        sub_channel = np.zeros((1, PubChem + PubChem_add))
        temp_channel = np.zeros((1, PubChem + PubChem_add))
        ...
        if len(value1) < 10:
            for i in range(len(value1)):
                ...
                PubChem_keys = PubChem_keys * coefficient
                temp_channel[0, 0:PubChem] = PubChem_keys 
                sub_channel += temp_channel

        else:
            sub_channel = sub_channel
        sub_channel = sub_channel.reshape((30, 30))
        sub_channel = np.insert(sub_channel, 0, ph, axis=0)
        sub_channel = np.insert(sub_channel, 0, ph, axis=1)
        sub_channel = sub_channel.reshape((1, 31 * 31))
    ...
    for key2, value2 in list(products_dict.items()):
        ...
        pro_channel = np.zeros((max, PubChem + PubChem_add))
        temp_channel = np.zeros((1, PubChem + PubChem_add))
        if len(value2) < 10:
            for i in range(len(value2)):
                ...
                PubChem_keys2 = PubChem_keys2 * coefficient
                temp_channel[0, 0:PubChem] = PubChem_keys2 
                pro_channel += temp_channel 
        else:
            pro_channel = pro_channel

        pro_channel = pro_channel.reshape((30, 30))
        pro_channel = np.insert(pro_channel, 0, ph, axis=0)
        pro_channel = np.insert(pro_channel, 0, ph, axis=1)
        pro_channel = pro_channel.reshape((1, 31 * 31))
