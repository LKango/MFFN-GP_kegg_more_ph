...
def generate_data(features,compounds):
    rootx = "./test1/data/sub_channel2/"
    rooty = "./test1/data/pro_channel2/"
    features1 = features
    ...
    for key1, value1 in list(substrates_dict.items()):
        sub_channel = np.zeros((1, FP4 + FP4_add))
        temp_channel = np.zeros((1, FP4 + FP4_add))

        if len(value1) < 10:
            for i in range(len(value1)):
                comp = list(value1.keys())[i] 
                fea = features1[comp].to_frame() 
                maccs_keys = fea.values[1188:1355,0] # MACCS fingerprint 167
                PubChem_keys = fea.values[0:881,0] # PubChem_keys fingerprint 881
                FP4_keys = fea.values[881:1188, 0]  # PubChem_keys fingerprint 881

                coefficient = list(value1.values())[i]
                FP4_keys = FP4_keys * coefficient

                temp_channel[0, 0:FP4] = FP4_keys 
                sub_channel += temp_channel

        else:
            sub_channel = sub_channel

        sub_channel = sub_channel.reshape((18, 18))
        sub_channel = np.insert(sub_channel, 0, ph, axis=0)
        sub_channel = np.insert(sub_channel, 0, ph, axis=1)
        sub_channel = sub_channel.reshape((1, 19 * 19))
    ...
    for key2, value2 in list(products_dict.items()):
        ...
        pro_channel = np.zeros((max, FP4 + FP4_add))
        temp_channel = np.zeros((1, FP4 + FP4_add))
        if len(value2) < 10:
            for i in range(len(value2)):
                ...
                FP4_keys2 = FP4_keys2 * coefficient
                temp_channel[0, 0:FP4] = FP4_keys2  
                pro_channel += temp_channel 

        else:
            pro_channel = pro_channel
        pro_channel = pro_channel.reshape((18, 18))
        pro_channel = np.insert(pro_channel, 0, ph, axis=0)
        pro_channel = np.insert(pro_channel, 0, ph, axis=1)
        pro_channel = pro_channel.reshape((1, 19 * 19))
...
if __name__ == '__main__':
    ...
    for ph in ph_list:
        ...
        compounds_df = pd.read_csv("./test1/compound_has_smiles.tsv", header=None)  # 18,309 compounds, training set
        features = pd.read_csv("./test1/features.csv", header=None)  # features.shape (18309, 1355) 
